...
const ScannerSales: React.FC = () => {
  ...
  const handleProcessSale = async () => {
    ...
    if (paymentType === 'cash' && paymentAmount !== null && paymentAmount < total) {
      toast({
    ...
    try {
      const sale = await SalesService.processSale({
        ...
        total,
        paymentType,
        paymentAmount: paymentType === 'cash' ? (paymentAmount || 0) : (paymentType === 'ewallet' ? total : 0),
        ...
        <div className="text-gray-800 p-4 flex-1 min-h-0 flex flex-col">
          ...
          {mode === 'scanner' ? (
            <div className="flex-1 overflow-y-auto">
              ...
              {/* Payment Input */}
              <div className="mt-4">
                <label className="block text-gray-800 text-sm font-medium mb-2">Payment Amount (₱)</label>
                <Input
                  type="number"
                  value={paymentAmount || ''}
